...
namespace ConsoleEngine.Services
{
    public static class Game
    {
        ...
        public static void Start()
        {
            ...
            if (Plr == null)
            {
                throw new Exception("There's no player registered in Game.Plr");
            }
            if (Scenes.Count == 0)
            {
                throw new Exception("There are no scenes registered in Game.Scenes");
            }
        ...
        public static async void EachSecondLoop()
        {
            while (true)
            {
                ...
                if (CurrentScene != null)
                {
                    ...
                    List<Entity> ignore = new List<Entity>();

                    // Entities
                    for (int i = 0; i < CurrentScene.Entities.Count; i++)
                    {
                        if (i >= CurrentScene.Entities.Count)
                            continue;

                        Entity e = CurrentScene.Entities[i];
        ...
        public static void ThrowIfNotCurrentScene(Scene s)
        {
            ...
            {
                throw new Exception(
                    $"Expected scene '{s.Description}' to be current scene, but current scene is actually '{CurrentScene.Description}'."
                );
            }
        }
    }
