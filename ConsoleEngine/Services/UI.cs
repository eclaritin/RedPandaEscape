...
namespace ConsoleEngine.Services
{
    public static class UI
    {
        ...
        private static int PrimaryColWidth
        {
            get { return Game.Size[0] / 2 - 1; }
        }
        private static int SecondaryColWidth
        {
            get { return Game.Size[0] - PrimaryColWidth - 2; }
        }
        ...
        public static class StoredState
        {
            ...
            public static void Save(bool overwrite = false)
            {
                ...
                {
                    throw new Exception(
                        "Stored state is already set! If you mean to overwrite, pass true to my overwrite argument!"
                    );
                }
        ...
        private static int SETSELECTION_LOOP(
            int n_choices,
            Action<int, int, int> renderSelectionState,
            Action<int>? onHover = null,
            bool StayHaltedFlag = false,
            ConsoleKey EscapeKey = ConsoleKey.Escape
        )
        {
            if (n_choices == 0)
                throw new Exception("Cannot start selection loop from empty array.");
            ...
            if (dispHeight > n_choices)
            {
                dispHeight = n_choices;
            }
            ...
            while (!chosen)
            {
                ...
                if ((choice + top) >= n_choices)
                {
                    choice = dispHeight - 1;
                    top = n_choices - dispHeight;
                    continue;
                }
        ...
        public static int DisplaySelectionScreen(
            string[] Choices,
            string Title = "",
            string Body = ""
        )
        ...
        public static void Clear()
        {
            ...
            {
                throw new Exception(
                    "Please specify column to clear (0: Primary, 1: Secondary, 2: Both)"
                );
            }
            ToWrite = "";
            Update();
        ...
        {
            if (IsDescription && col != 2)
            {
                throw new Exception("Cannot clear specific col when UI is in Description mode");
            }
            if (IsDescription)
            {
                Clear();
        ...
        ) // Description mode
        {
            ...
            if (!IsDescription)
            {
                throw new Exception("Please specify which column to set selection.");
            }

            return SETSELECTION_LOOP(
                choices.Length,
                (int top, int choice, int dispHeight) =>
                {
                    // vars
                    string printable = "";
        ...
        ) // TwoCols mode
        {
            ...
            if (IsDescription && col != 2)
            {
                throw new Exception("Cannot write to column while UI is set to description mode.");
            }
            if (IsDescription)
            {
