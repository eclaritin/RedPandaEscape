...
namespace ConsoleEngine.Services
{
  public static class Render
  {
    ...
    {
      if (Game.CurrentScene != null) { NextScene = ToRender; return; }
      UI.StoredState.Construct();
      Game.CurrentScene = ToRender;
      Game.CurrentScene.Visible = true;
      UI.SetToDescription();
      Game.CurrentScene.OnRender?.Invoke();
      foreach (var t in Game.CurrentScene.Matrix) { t.OnSceneLoad?.Invoke(t); }
      foreach (var e in Game.CurrentScene.Entities) { e.OnSceneLoad?.Invoke(e); }
